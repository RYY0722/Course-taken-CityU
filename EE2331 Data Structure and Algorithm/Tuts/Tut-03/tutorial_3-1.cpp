...
void longest_block(const int* a, int n, int& start, int& len)
{
	...
	while (!done)
	{
		...
		if (j >= n) {
			...
			j %= n;
		}//if j >= n, after this round of searching, no need to count any more
		//otherwise in the next round, i will start at [0,n-1] again, which has been done already.
		while (a[j] == a[i]) {
			j++;
			if (j - i == n) {
				done = true;//have reached the max of possible length
				break;
			}
		...
		if ((j - i + n) % n> len || j - i >len){
			start = i;
			len = j - i > len ? j - i : (j - i + n) % n;
		}
...
void longestSubSeq_NoDup(const int* a, int n, int& start, int& len)
{
	...
	int j = 0;
	bool isSecond = false;
//judge if it is the second time of scanning the array in terms of i
	while (!done)
	{
		j = i - 1;
		...
		if ((i < j &&  j < left)||(j < left && left <= i)) {//no duplicate element on the "left"
			int temp = i - left + 1 == n ? n : (i - left + 1 + n) % n;
			if (temp > len ) {
				start = left;
				len = temp;
			}
		...
		if(!isSecond)isSecond = i == 0;
		if (isSecond && left < start )break;
//left < start and isSecond means that counting starts between 0 and start again
